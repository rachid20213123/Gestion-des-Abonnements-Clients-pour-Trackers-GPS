...
interface EditInterventionTypeModalProps {
  type: {
    id: number;
    name: string;
    description: string;
...
export function EditInterventionTypeModal({ type, onClose }: EditInterventionTypeModalProps) {
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: type
  });
  const { editInterventionType } = useStore();

  const onSubmit = (data: any) => {
    editInterventionType(type.id, {
      ...data,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-md p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">Modifier le Type d'intervention</h3>
